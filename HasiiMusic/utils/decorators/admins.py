        ...
        if await is_maintenance() is False:
            if message.from_user.id not in SUDOERS:
                return await message.reply_text(
                    text=f"{app.mention} Đang được bảo trì, hãy truy cập <a href={SUPPORT_CHAT}>nhóm hỗ trợ</a> để biết lý do.",
                ...
                if not admins:
                    ...
                    if message.from_user.id not in admins:
                        if await is_skipmode(message.chat.id):
                            ...
                            text = f"""<b>Cần quyền quản trị viên</b>

Làm mới bộ nhớ đệm quản trị viên bằng cách : /reload

» {upvote} Cần đủ số phiếu bầu để thực hiện hành động này."""
        ...
        if await is_maintenance() is False:
            if CallbackQuery.from_user.id not in SUDOERS:
                return await CallbackQuery.answer(
                    f"{app.mention} Đang được bảo trì, hãy truy cập nhóm hỗ trợ để biết lý do.",
                    show_alert=True,
                )
        try:
